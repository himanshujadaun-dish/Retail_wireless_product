# ------------------------------------------------------------
# Wireless Cortex AI v5.6.2 — Stable Streamlit 1.39+ (Toggle + FAQ Fix)
# ------------------------------------------------------------
...
with st.sidebar:
    ...
    st.markdown("---")
    st.caption("**Wireless Cortex AI v5.6.2 | Stable Fix (Toggle + FAQ)**")
...
def sales_answers(q):
    if q=="Show me sales trends by channel.": return f"📈 Indirect {fmt_pct(random.uniform(8,15))} MoM, Web {fmt_pct(random.uniform(5,12))}."
    if q=="What were the top-selling devices last month?": return "📊 iPhone 16 Pro Max and Samsung A15 led overall."
    if q=="Compare iPhone vs Samsung sales this quarter.": return "📊 iPhone 55% vs Samsung 40% share."
    if q=="Which SKUs have the highest return rate?": return "⚠️ Moto G Stylus showing elevated 4.2% return rate."
    if q=="What are the sales forecasts for next month?": return "🔮 +6% MoM expected, mainly from iPhone 16 upgrades."
    return None
def inventory_answers(q):
    if q=="Which SKUs are low in stock?": return "🏭 iPhone 16 128GB and A15 Blue are low in West Coast DCs."
    if q=="Show inventory aging by warehouse.": return "⏳ Denver 30d, Dallas 26d, NY 20d."
    if q=="How many iPhone 16 units are in Denver DC?": return f"📦 {fmt_int(1200,1800)} units."
    if q=="List SKUs with overstock conditions.": return "⚠️ Moto G Power overstocked in Central."
    if q=="What's the daily inventory update feed?": return "🕓 Dataiku job INV_2025_09 every 4h."
    return None
def shipments_answers(q):
    if q=="Show delayed shipments by DDP.": return "🚚 Brightstar delays in Midwest due to weather."
    if q=="How many units shipped this week?": return f"📦 {fmt_int(17000,19000)} units WTD."
    if q=="Which SKUs are pending shipment confirmation?": return f"⚙️ {fmt_int(180,260)} pending (60% Apple)."
    if q=="Track shipment status for iPhone 16 Pro Max.": return "📍 Dallas TX, ETA 2 days."
    if q=="List DDPs with recurring delays.": return "⏰ Brightstar, Marceco recurring delays >5x Q3."
    return None
def pricing_answers(q):
    if q=="Show current device pricing by channel.": return "💰 iPhone 16: $1,099 (Web), $1,049 (Indirect)."
    if q=="Which SKUs had price drops this week?": return "📉 A15 (-$30), Moto G (-$25)."
    if q=="Compare MSRP vs promo prices.": return "💵 Avg 9% below MSRP."
    if q=="Show competitor pricing insights.": return "🏷️ Metro is $20 lower on iPhone 15."
    if q=="What’s the margin for iPhone 16 Pro Max?": return f"💸 Margin ≈ {fmt_pct(random.uniform(11,13))}."
    return None
def forecast_answers(q):
    if q=="Show activation forecast by SKU.": return f"🔮 iPhone 16: {fmt_int(23000,26000)}, A15: {fmt_int(17500,20000)}."
    if q=="Compare actual vs forecast for Q3.": return f"📊 Accuracy ≈ {fmt_pct(random.uniform(90,92))}."
    if q=="Which SKUs are forecasted to grow fastest?": return "🚀 A15 and Moto G Stylus +15%."
    if q=="Show forecast accuracy trend by month.": return "📈 86% → 89% → 91% (Jul–Sep)."
    if q=="Update forecast model inputs from Dataiku.": return "⚙️ Inputs refreshed 01:15 AM MT."
    return None
def answer_for_question(q):
    ...
    return a if a else "⚠️ Limited Data — working on getting in more data sources"

# ✅ Re-add FAQ dictionary (was missing before)
FAQ={
    "Sales":[
