# ------------------------------------------------------------
# Wireless Cortex AI v5.6.3 — Stable Streamlit 1.39+ (Dropdown + Toggle Fix)
# ------------------------------------------------------------
...
max-width:80%;box-shadow:0 2px 6px rgba(0,0,0,.25);}}
.chat-bubble-ai{{background-color:{ai};color:#E8EEF7;
padding:10px 14px;border-radius:15px;margin:8px 0;
max-width:80%;box-shadow:0 2px 6px rgba(0,0,0,.15);}}
</style>
...
with st.sidebar:
    ...
    st.markdown("---")
    st.caption("**Wireless Cortex AI v5.6.3 | Stable Fix (Dropdown + Toggle)**")
...
def answer_for_question(q):
    qn = q.lower().strip()
    ans = (
        sales_answers(qn) or inventory_answers(qn) or shipments_answers(qn)
